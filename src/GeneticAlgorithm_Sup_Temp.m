...
%%(smallest ratio rhoprime/rhocircle) under temperature initialization
...
for nn = 1:n
    ...
    while ii <= m
        ...
        if ~any(~(ones(1,N)*G(:,:,ii)))  %checks that no vertices are isolated
            [rho0(ii), rhoprime(ii), isocheck] = WeakSelectionFixProb(N, G(:,:,ii),1);
...
title(strcat('Smallest \rho''/\rho^\circ for N = ',num2str(N),' is ', num2str(val)));
