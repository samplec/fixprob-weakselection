%%This script uses a genetic algorithm to find the best amplifier 
%%(largest rho prime) under temperature initialization
...
while i<=m %creates m graphs
    ...
    if ~any(~(G(:,:,i)*ones(N,1))) %checks that the created graph has no isolated vertices
        [rho0(i), rhoprime(i), isocheck] = WeakSelectionFixProb(N, G(:,:,i),1);  %calcuate weak selection fixation probabity
...
for nn = 1:n
    ...
    while ii <= m
        ...
        if ~any(~(ones(1,N)*G(:,:,ii)))  %checks that no vertices are isolated
            [rho0(ii), rhoprime(ii), iso(ii)] = WeakSelectionFixProb(N, G(:,:,ii),1);
            if iso(ii) < 2  %graph is connected and fix prob can be calculated
                ii = ii + 1;
            end
        end
    end
...
title(strcat('Largest \rho'' for N = ',num2str(N),' is ', num2str(val)));
